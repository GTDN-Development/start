...
const representativeScenarios = [
  {
    ...
    flow: "settings/page.tsx -> workspace-general-settings-section.tsx -> workspace-name/url/avatar-settings-item.tsx -> workspace-general-actions.ts -> workspace-general-service.ts",
    why: "Tracks the final route handoff into one general-settings owner and the remaining workspace mutation boundary.",
    files: [
  ...
  },
  {
    ...
    flow: "settings/members/page.tsx -> workspace-members-settings-section.tsx -> workspace-members-table.tsx / workspace-members-management-dialogs.tsx -> workspace-members-actions.ts -> workspace-members-service.ts",
    why: "Tracks the final members surface where one client owner drives role, removal, and leave flows without an extra management wrapper.",
    files: [
  ...
  },
  {
    ...
    flow: "account/page.tsx -> application-root.tsx -> account-profile-context.tsx -> avatar/display-name-settings-item.tsx -> account-profile-actions.ts -> account-profile-service.ts",
    why: "Provides a control case where the shared profile snapshot still updates through one explicit client snapshot boundary.",
    files: [
  ...
  },
  {
    ...
    flow: "account/security/page.tsx -> your-devices-settings-item.tsx -> device-session-actions.ts -> current-user.ts -> device-sessions-service.ts",
    why: "Tracks the contracted device-session path where the page loads directly and auth-owned current-user logic fronts low-level session operations.",
    files: [
