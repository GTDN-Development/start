...
8. Signed-in user opening sign-in or sign-up is redirected to effective app entry.
9. User creates a workspace and lands on its overview as owner.
10. Member opens the members page, sees roster and roles, and does not see pending invites or management controls.
11. Admin invites a user, invited user follows the link while signed out, signs in, accepts, and lands on workspace overview.
12. Invited user signs in with the wrong email and sees the account-mismatch state.
13. Owner promotes another user to owner, then the original owner can leave successfully.
14. Final remaining owner attempts to leave and is blocked.
15. User opens an inaccessible workspace URL and gets workspace-scoped 404.
...
11. Owner can promote another user to owner and multiple owners can coexist.
12. Final-owner downgrade, removal, or leave is blocked.
13. Members-page data for regular members excludes pending invites.
14. Invite acceptance requires both verified session and matching email.
15. Active workspace is cleared or ignored after leave, deletion, or external loss of access.
16. Account deletion is blocked while the user remains the final owner of any workspace.
17. Account deletion removes the user from shared workspaces when deletion is allowed.
