...
import {
  removeSettingsAvatarAction,
  uploadSettingsAvatarAction,
...
export function SettingsAvatarSettingsItem() {
  const t = useTranslations("pages.settings");
  const { profile, patchProfile, isAvatarUpdating, setIsAvatarUpdating } = useSettingsProfile();
  ...
  async function handleAvatarInputChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const preparedAvatarFileResult = await prepareAvatarUpload(avatarFile, {
        maxFileSizeBytes: MAX_SETTINGS_AVATAR_SIZE_BYTES,
      });
      ...
      const response = await runAsyncTransition(() => uploadSettingsAvatarAction(avatarFormData));
  ...
  async function handleAvatarRemoveClick() {
    ...
    const response = await runAsyncTransition(() => removeSettingsAvatarAction());
