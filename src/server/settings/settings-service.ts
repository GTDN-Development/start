...
const MAX_SETTINGS_AVATAR_SIZE_BYTES = settingsConfig.limits.avatarMaxSizeBytes;
...
export async function updateCurrentUserProfileName(
  name: string
): Promise<ServerAuthResponse<SettingsProfilePayload>> {
  const normalizedName = normalizeProfileName(name);

  if (normalizedName && normalizedName.length > MAX_SETTINGS_PROFILE_NAME_LENGTH) {
    return {
      ok: false,
  ...
  try {
    ...
    return {
      ok: true,
      data: {
        profile: createSettingsProfileSnapshot(currentUser.pb, updatedRecord),
      },
    };
  } catch (error) {
    ...
    if (errorCode === "UNKNOWN_ERROR") {
      logSettingsServiceError("updateCurrentUserProfileName", error);
    }
...
export async function updateCurrentUserAvatar(
  ...
  if (avatarFile.size > MAX_SETTINGS_AVATAR_SIZE_BYTES) {
    return {
      ok: false,
      errorCode: "VALIDATION_ERROR",
    };
  ...
  try {
    ...
    if (errorCode === "UNKNOWN_ERROR") {
      logSettingsServiceError("updateCurrentUserAvatar", error);
    }
...
export async function removeCurrentUserAvatar(): Promise<
  ServerAuthResponse<SettingsProfilePayload>
> {
  const currentUser = await requireCurrentUser();
  ...
  try {
    ...
    if (errorCode === "UNKNOWN_ERROR") {
      logSettingsServiceError("removeCurrentUserAvatar", error);
    }
...
export async function requestEmailChangeForCurrentUser(
  ...
  try {
    ...
    if (errorCode === "UNKNOWN_ERROR") {
      logSettingsServiceError("requestEmailChangeForCurrentUser", error);
    }
...
export async function deleteCurrentUserAccountWithPassword(
  ...
  try {
    ...
    if (errorCode === "UNKNOWN_ERROR") {
      logSettingsServiceError("deleteCurrentUserAccountWithPassword.verifyPassword", error);
    }
  ...
  try {
    ...
    if (errorCode === "UNKNOWN_ERROR") {
      logSettingsServiceError("deleteCurrentUserAccountWithPassword.delete", error);
    }
...
export async function updateCurrentUserPassword(input: {
  ...
  try {
    ...
    if (errorCode === "UNKNOWN_ERROR") {
      logSettingsServiceError("updateCurrentUserPassword", error);
    }
...
function createSettingsProfileSnapshot(pb: PocketBase, record: UsersRecord) {
  return {
    id: record.id,
    email: record.email,
...
function logSettingsServiceError(context: string, error: unknown) {
  logServiceError("settings-service", context, error);
}
